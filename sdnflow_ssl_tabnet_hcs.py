...
# =========================================================
# 0) PATCH TABNET UNSUPERVISED VALIDATION METRIC
# =========================================================
...
BASE_DIR = os.path.join(PROJECT_ROOT, "data", "sdnflow")
...
OUTPUT_DIR = os.path.join(PROJECT_ROOT, "outputs", "sdnflow")
...
OPEN_LABELS_ORDER = ["Normal", "ddos", "probe", NEW_ATTACK_LABEL]
...
def assert_same_feature_layout(
    ...
    missing = [c for c in reference_cols if c not in current_cols]
...
def normalize_open_labels(labels: np.ndarray) -> np.ndarray:
    labels = np.asarray(labels).astype(str)
    return np.where(labels == "Unknown", NEW_ATTACK_LABEL, labels)
...
def compute_hybrid_scores(
    ...
    proto_cls = np.array([class_names[i] for i in proto_idx])
    clf_cls = np.array([class_names[i] for i in clf_idx])
    ...
    return (
        ...
        np.array(hybrid_scores, dtype=np.float32)
    )
...
y_test_open_text = normalize_open_labels(open_test_df[OPENSET_COL].astype(str).values)


# =========================================================
# 6) CLIPPING AND SCALING
# =========================================================
...
joblib.dump(
    {"low": clip_low, "high": clip_high},
    os.path.join(OUTPUT_DIR, "clip_bounds.pkl")
)
...
model_path = os.path.join(OUTPUT_DIR, "tabnet_sdnflow_model")
clf.save_model(model_path)


# =========================================================
# 10) CLOSED-SET VALIDATION METRICS
# =========================================================
...
# =========================================================
# 11) LATENT REPRESENTATION EXTRACTION
# =========================================================
print("\n" + "=" * 80)
...
# =========================================================
# 12) PROTOTYPES AND VALIDATION THRESHOLDS
# =========================================================
...
# =========================================================
# 13) HYBRID THRESHOLD CALIBRATION
# =========================================================
...
val_pred_open = np.array([
    NEW_ATTACK_LABEL if val_hybrid_scores[i] > hybrid_thresholds[val_proto_cls[i]]
    else val_proto_cls[i]
    for i in range(len(val_proto_cls))
])

val_reject_ratio = np.mean(val_pred_open == NEW_ATTACK_LABEL)
...
test_pred_open = np.array([
    NEW_ATTACK_LABEL if test_hybrid_scores[i] > hybrid_thresholds[test_proto_cls[i]]
...
metrics_summary = {
    ...
    "new_attack_label": NEW_ATTACK_LABEL,
